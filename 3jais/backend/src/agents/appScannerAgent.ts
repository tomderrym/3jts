 ...
 */
export async function runAppScannerAgent(workspaceRoot: string): Promise<Suggestion[]> {
  const suggestions: Suggestion[] = [];
  const now = new Date().toISOString();

  const tree = (await invokeTool("scan_workspace", { root: workspaceRoot, maxDepth: 2 }, workspaceRoot)) as {
    path: string;
    type: string;
  }[];
  ...
  try {
    const readme = await invokeTool(
      "read_file",
      { path: "README.md", maxBytes: 4000 },
      workspaceRoot
    );
