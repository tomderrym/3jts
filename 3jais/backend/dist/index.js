...
import { WORKSPACE_ROOT, getConstraints, getFlow, getGoals, getRankedVault, saveFlow, setSuggestionStatus, upsertSuggestions, } from "./state.js";
const app = express();
...
app.post("/api/tools/invoke", async (req, res) => {
    try {
        ...
        const out = await invokeTool(name, args ?? {}, WORKSPACE_ROOT);
...
app.post("/api/suggestions/:id/approve", (req, res) => {
    ...
    if (!gate.ok)
        return res.status(403).json({ error: "policy blocked", reasons: gate.reasons });
