/**
 * Block ID: bookingsscreen
 * Category: component
 * Description: BookingsScreen component
 * Runtime: Glue-Paste Safe (no imports, no JSX, no framework)
 */

(function () {
  // ---- Guard: prevent double definition ----
  if (window.__BLOCK_BOOKINGSSCREEN_DEFINED__) return;
  window.__BLOCK_BOOKINGSSCREEN_DEFINED__ = true;
  ...
  function mount(props = {}, host) {
    ...
    root.textContent = "BookingsScreen component (basic conversion - JSX not fully parsed)";
  ...
  if (window.BlockRegistry && typeof window.BlockRegistry.register === "function") {
    window.BlockRegistry.register("bookingsscreen", {
      mount,
      unmount,
    });
  } else {
