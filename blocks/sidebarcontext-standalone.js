/**
 * Block ID: sidebarcontext
 * Category: component
 * Description: SidebarContext component
 * Runtime: Glue-Paste Safe (no imports, no JSX, no framework)
 */

(function () {
  // ---- Guard: prevent double definition ----
  if (window.__BLOCK_SIDEBARCONTEXT_DEFINED__) return;
  window.__BLOCK_SIDEBARCONTEXT_DEFINED__ = true;
  ...
  function mount(props = {}, host) {
    ...
    root.textContent = "SidebarContext component (basic conversion - JSX not fully parsed)";
  ...
  if (window.BlockRegistry && typeof window.BlockRegistry.register === "function") {
    window.BlockRegistry.register("sidebarcontext", {
      mount,
      unmount,
    });
  } else {
