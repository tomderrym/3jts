/**
 * Block ID: mechanicprofilescreen
 * Category: component
 * Description: MechanicProfileScreen component
 * Runtime: Glue-Paste Safe (no imports, no JSX, no framework)
 */

(function () {
  // ---- Guard: prevent double definition ----
  if (window.__BLOCK_MECHANICPROFILESCREEN_DEFINED__) return;
  window.__BLOCK_MECHANICPROFILESCREEN_DEFINED__ = true;
  ...
  function mount(props = {}, host) {
    ...
    root.textContent = "MechanicProfileScreen component (basic conversion - JSX not fully parsed)";
  ...
  if (window.BlockRegistry && typeof window.BlockRegistry.register === "function") {
    window.BlockRegistry.register("mechanicprofilescreen", {
      mount,
      unmount,
    });
  } else {
