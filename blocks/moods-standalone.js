 ...
 * Block ID: moods
 * Category: component
 * Description: MOODS component
 * Runtime: Glue-Paste Safe (no imports, no JSX, no framework)
 */

(function () {
  // ---- Guard: prevent double definition ----
  if (window.__BLOCK_MOODS_DEFINED__) return;
  window.__BLOCK_MOODS_DEFINED__ = true;
  ...
  function mount(props = {}, host) {
    ...
    root.textContent = "MOODS component (basic conversion - JSX not fully parsed)";
  ...
  if (window.BlockRegistry && typeof window.BlockRegistry.register === "function") {
    window.BlockRegistry.register("moods", {
      mount,
      unmount,
    });
  } else {
