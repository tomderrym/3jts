 ...
 * Block ID: appcontext
 * Category: component
 * Description: AppContext component
 * Runtime: Glue-Paste Safe (no imports, no JSX, no framework)
 */

(function () {
  // ---- Guard: prevent double definition ----
  if (window.__BLOCK_APPCONTEXT_DEFINED__) return;
  window.__BLOCK_APPCONTEXT_DEFINED__ = true;
  ...
  function mount(props = {}, host) {
    ...
    const user = props.user !== undefined ? props.user : undefined;

    // ---- Root ----
    root = document.createElement("div");
    root.style.padding = "2rem";
    root.style.fontFamily = "system-ui, sans-serif";
    root.textContent = "AppContext component (basic conversion - JSX not fully parsed)";
  ...
  if (window.BlockRegistry && typeof window.BlockRegistry.register === "function") {
    window.BlockRegistry.register("appcontext", {
      mount,
      unmount,
    });
  } else {
