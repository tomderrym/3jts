/**
 * Block ID: signupscreen
 * Category: component
 * Description: SignupScreen component
 * Runtime: Glue-Paste Safe (no imports, no JSX, no framework)
 */

(function () {
  // ---- Guard: prevent double definition ----
  if (window.__BLOCK_SIGNUPSCREEN_DEFINED__) return;
  window.__BLOCK_SIGNUPSCREEN_DEFINED__ = true;
  ...
  function mount(props = {}, host) {
    ...
    root.textContent = "SignupScreen component (basic conversion - JSX not fully parsed)";
  ...
  if (window.BlockRegistry && typeof window.BlockRegistry.register === "function") {
    window.BlockRegistry.register("signupscreen", {
      mount,
      unmount,
    });
  } else {
