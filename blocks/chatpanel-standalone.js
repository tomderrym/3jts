...
export default function ChatPanel: React.FC<ChatPanelProps> = ({ 
  ...
  return (
    <div className="flex flex-col border-r border-white/5 bg-[#09090b] h-full relative z-20 shadow-2xl" style={{ width: style?.width || '350px', ...style }}>
      {/* Header */}
      <div className="h-14 px-4 border-b border-white/5 flex items-center justify-between bg-[#0c0c0e]/80 backdrop-blur-md shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          {agentStatus === 'paused' && (
             <div className="px-2 py-0.5 rounded-full bg-slate-500/10 border border-slate-500/20 text-[9px] font-bold text-slate-400">
               PAUSED
             </div>
          )}
      ...
      <>
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
            ...
            <div className="h-full flex flex-col items-center justify-center opacity-40 select-none">
                ...
                <p className="text-xs font-bold uppercase tracking-widest text-slate-500">Ready to Build</p>
            </div>
            )}
            ...
            <div key={i} className={`flex flex-col gap-2 ${msg.role === 'user' ? 'items-end' : 'items-start'} animate-in slide-in-from-bottom-2 duration-300`}>
                ...
                <div className="flex items-center gap-2">
                    {msg.role === 'user' ? (
                    <>
                        <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">You</span>
                        <User size={10} />
                    </>
                    ) : (
                    <>
                        <Bot size={10} />
                        <span className="text-[10px] font-bold uppercase tracking-wider text-indigo-400">Architect</span>
                    </>
                    )}
                </div>
                </div>
                ...
                {msg.tokenUsage && (
                    ...
                    <span className="text-indigo-400">IN: {msg.tokenUsage.promptTokens}</span>
                    <span className="text-emerald-400">OUT: {msg.tokenUsage.responseTokens}</span>
                    <span className="font-bold text-slate-300">Σ {msg.tokenUsage.totalTokens}</span>
                    </div>
                )}
                </div>
                ...
                <div className="max-w-[90%] flex flex-wrap gap-2 mt-1 pl-1">
                    ...
                    if (chunk.web?.uri) {
                        ...
                        <a 
                            ...
                            className="group flex items-center gap-1.5 px-2.5 py-1.5 bg-[#121214] hover:bg-indigo-900/20 border border-white/5 hover:border-indigo-500/30 rounded-md text-[10px] text-slate-400 hover:text-indigo-300 transition-all no-underline"
                        >
                            <Globe size={10} className="group-hover:text-indigo-400 transition-colors" />
                            <span className="truncate max-w-[180px] font-medium">{chunk.web.title || new URL(chunk.web.uri).hostname}</span>
                        </a>
                        );
                    }
                    return null;
            ...
            <div className="flex flex-col gap-2 items-start animate-in fade-in duration-300">
                <div className="flex items-center gap-2 opacity-50 px-1">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-wider text-indigo-400">
                        {agentStatus === 'planning' ? 'Planning Phases...' : 'Executing Phase...'}
                    </span>
                </div>
        ...
        <div className="p-4 bg-[#0c0c0e] border-t border-white/5">
            <div className="flex items-center justify-between mb-2 px-1">
                <div className="flex items-center gap-3">
                    ...
                    >
                        <Settings2 size={10} />
                        <span className="uppercase">{activeModel.replace('gemini-', '').replace('claude-', '').split('-')[0]}</span>
                    </button>
                    
                    <div className="flex items-center gap-1 text-[9px] font-mono text-slate-600">
                        <Calculator size={10} />
                        <span>EST: {estimatedTokens ? estimatedTokens : '-'} Tok</span>
                    </div>
                </div>
                ...
                <div className="mb-3 grid grid-cols-3 gap-2">
                    ...
                    {uploadedVideos.map((vid, idx) => (
                        <div key={`vid-${idx}`} className="relative aspect-square group rounded-lg overflow-hidden border border-white/10 bg-black flex items-center justify-center">
                            <video src={vid} controls={false} className="w-full h-full object-cover" />
                            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                <button 
                            ...
                            <Video size={24} className="absolute text-white/70 pointer-events-none" />
                        </div>
                    ))}
          ...
          <TaskQueue tasks={tasks} taskGroups={taskGroups} onDelete={onDeleteTask} onAdd={onAddTask} />
      )}
    </div>
  );
};
