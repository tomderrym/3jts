...
export default function TaskTokenSidebar: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col bg-slate-950 text-xs md:text-sm border-l border-slate-800">
      <div className="px-3 py-2 border-b border-slate-800 flex items-center justify-between bg-slate-900/80">
        <div>
          <div className="font-semibold text-slate-100 text-xs">Task Token Usage</div>
          <div className="text-[10px] text-slate-400">Aggregated from TaskRun.token_usage</div>
        </div>
      </div>
      ...
      <div className="p-3 border-b border-slate-800 text-[11px] grid grid-cols-3 gap-2">
        <div>
          ...
          <div className="font-semibold text-slate-50">{totalTokens.toLocaleString()}</div>
        </div>
        <div>
          <div className="text-slate-400">Runs</div>
          <div className="font-semibold text-slate-50">{runCount}</div>
        </div>
        <div>
          <div className="text-slate-400">Avg / run</div>
          <div className="font-semibold text-slate-50">{avgTokens.toLocaleString()}</div>
        </div>
      </div>

      <div className="flex-1 overflow-auto">
        {loading && (
          <div className="p-3 text-slate-400 text-[11px]">Loading recent runs...</div>
        )}
        {error && !loading && (
          <div className="p-3 text-red-300 text-[11px]">{error}</div>
        )}
        {!loading && !error && runs.length === 0 && (
          <div className="p-3 text-slate-400 text-[11px]">No recent task runs.</div>
        )}
        {!loading && !error && runs.length > 0 && (
            ...
            {runs.map((r, idx) => (
              <li key={r.id ?? `${r.task_id}-${idx}`} className="px-3 py-2 text-[11px] flex flex-col gap-0.5">
                <div className="flex items-center justify-between">
                  <span className="font-medium text-slate-100">{r.model}</span>
                  <span
                ...
                <div className="flex items-center justify-between text-slate-400">
                  <span>Tokens</span>
                  <span className="font-mono text-slate-100">{(r.token_usage ?? 0).toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between text-slate-500">
                  <span className="truncate mr-2">task: {r.task_id}</span>
                  <span>{r.created_at?.slice(11, 19)}</span>
                </div>
                {r.error_message && (
                  <div className="text-[10px] text-amber-300">{r.error_message}</div>
                )}
              </li>
            ))}
          </ul>
