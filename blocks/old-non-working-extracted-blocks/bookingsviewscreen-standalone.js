...
const BookingsViewScreen = ({
  ...
  return createElement('div', {className: 'flex flex-col items-stretch p-0 w-full h-full overflow-y-auto'}, 'createElement('HeaderBar', {title: 'Bookings View'})
      ...
      <div className="w-full max-w-2xl mx-auto space-y-4 px-4 py-4">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-10 text-center text-slate-400 space-y-3">
            ...
            createElement('p', {className: 'font-medium text-slate-200'}, 'No bookings yet')
            createElement('p', {className: 'text-sm max-w-sm'}, 'New customer appointments will appear here once they are requested.')') : (
          <Card>
            <div className="space-y-4 animate-[fadeIn_220ms_ease-out]">
              {bookings.map((booking) => (
                <div key={booking.id} className="p-4 bg-slate-800/80 rounded-lg shadow-sm border border-slate-700">
                  ...
                  <p className="text-slate-300 text-sm flex items-center mb-1">createElement('Car', {className: 'mr-2'})Mechanic: {getDisplayName(booking.mechanicEmail)}</p>
                  <p className="text-slate-300 text-sm flex items-center mb-1">createElement('CalendarDays', {className: 'mr-2'})Date: {booking.date}</p>
                  <p className="text-slate-300 text-sm flex items-center">createElement('Clock', {className: 'mr-2'})Time: {booking.time}</p>
                </div>
              ))}
            </div>
          </Card>
