      ...
      <header className="bg-primary text-primary-foreground px-4 pt-8 pb-6">
        <div className="max-w-screen-xl mx-auto">
          <div className="flex items-center gap-3 mb-4">
            ...
            createElement('h1', {className: 'text-2xl flex-1'}, 'Book Service')
          </div>

          createElement('Progress', {className: 'h-2'})
          createElement('p', {className: 'text-sm mt-2 text-primary-foreground/80'}, 'Step {step} of 5')
        </div>
      </header>
      ...
      <div className="max-w-screen-xl mx-auto px-4 py-6">
            ...
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                <Avatar className="h-12 w-12">
                  ...
                  createElement('AvatarFallback', null, '{mechanic.full_name?.charAt(0) || 'M'}')
                </Avatar>
                <div>
                  createElement('p', {className: 'font-medium'}, '{mechanic.full_name}')
                  createElement('p', {className: 'text-sm text-muted-foreground'}, '{mechanic.mechanic_profiles?.specialties}')
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                createElement('Wrench', {className: 'h-5 w-5'})
                Select Service
              </CardTitle>
              createElement('CardDescription', null, 'Choose a service or describe what you need')
            </CardHeader>
            <CardContent className="space-y-4">
                ...
                <RadioGroup value={selectedServiceId} onValueChange={setSelectedServiceId}>
                  {services.map((service) => (
                    <div key={service.id} className="flex items-center space-x-2">
                      createElement('RadioGroupItem', null)
                      <Label htmlFor={service.id} className="flex-1 cursor-pointer">
                        <div className="flex justify-between items-start">
                          <div>
                            createElement('p', {className: 'font-medium'}, '{service.service_name}')
                            {service.service_description && (
                              createElement('p', {className: 'text-sm text-muted-foreground'}, '{service.service_description}')
                            )}
                          </div>
              ...
              <div className="space-y-2">
                createElement('Label', {htmlFor: 'customService'}, 'Or describe your custom service')
                <Input
                  id="customService"
        ...
        {step === 2 && (
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                createElement('MapPin', {className: 'h-5 w-5'})
                Service Location
              </CardTitle>
              createElement('CardDescription', null, 'Where should the service take place?')
            </CardHeader>
            <CardContent className="space-y-4">
              <RadioGroup value={serviceLocation} onValueChange={(value: any) => setServiceLocation(value)}>
                <div className="flex items-center space-x-2">
                  createElement('RadioGroupItem', {value: 'customer_site', id: 'customer_site'})
                  createElement('Label', {htmlFor: 'customer_site'}, 'My Location (Mobile Service)')
                </div>
                <div className="flex items-center space-x-2">
                  createElement('RadioGroupItem', {value: 'mechanic_workshop', id: 'mechanic_workshop'})
                  createElement('Label', {htmlFor: 'mechanic_workshop'}, 'Mechanic's Workshop')
                </div>
                <div className="flex items-center space-x-2">
                  createElement('RadioGroupItem', {value: 'custom', id: 'custom'})
                  createElement('Label', {htmlFor: 'custom'}, 'Custom Address')
                </div>
              </RadioGroup>

              {serviceLocation === 'custom' && (
                <div className="space-y-2">
                  createElement('Label', {htmlFor: 'customAddress'}, 'Enter Address')
                  <Input
                    id="customAddress"
        ...
        {step === 3 && (
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                createElement('Calendar', {className: 'h-5 w-5'})
                Date & Time
              </CardTitle>
              createElement('CardDescription', null, 'When would you like the service?')
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                createElement('Label', {htmlFor: 'bookingDate'}, 'Date')
                <Input
                  id="bookingDate"
                  type="date"
                  value={bookingDate}
              ...
              <div className="space-y-2">
                createElement('Label', {htmlFor: 'bookingTime'}, 'Time')
                <Input
                  id="bookingTime"
                  type="time"
                  value={bookingTime}
        ...
        {step === 4 && (
          <Card>
            <CardHeader>
              createElement('CardTitle', null, 'Additional Details')
              createElement('CardDescription', null, 'Help the mechanic prepare for your service')
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                createElement('Label', {htmlFor: 'vehicleInfo'}, 'Vehicle Information (Optional)')
                <Input
                  id="vehicleInfo"
              ...
              <div className="space-y-2">
                createElement('Label', {htmlFor: 'specialInstructions'}, 'Special Instructions (Optional)')
                <Textarea
        ...
        {step === 5 && (
          <Card>
            <CardHeader>
              ...
              createElement('CardDescription', null, 'Please review your booking details')
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-3">
                <div className="flex justify-between">
                  createElement('span', {className: 'text-muted-foreground'}, 'Service:')
                  createElement('span', {className: 'font-medium'}, '{selectedService?.service_name || customServiceName}')
                </div>

                <div className="flex justify-between">
                  createElement('span', {className: 'text-muted-foreground'}, 'Location:')
                  createElement('span', {className: 'font-medium'}, '{serviceLocation === 'customer_site'
                      ? 'My Location'
                ...
                <div className="flex justify-between">
                  createElement('span', {className: 'text-muted-foreground'}, 'Date & Time:')
                  createElement('span', {className: 'font-medium'}, '{new Date(`${bookingDate}T${bookingTime}`).toLocaleString()}')
                </div>

                {vehicleInfo && (
                  <div className="flex justify-between">
                    createElement('span', {className: 'text-muted-foreground'}, 'Vehicle:')
                    createElement('span', {className: 'font-medium'}, '{vehicleInfo}')
                  </div>
                )}
                ...
                <div className="border-t pt-3 mt-3">
                  <div className="flex justify-between items-center">
                    ...
                    createElement('span', {className: 'text-2xl font-bold text-primary'}, '${totalAmount.toFixed(2)}')
                  </div>
                </div>
              </div>
            </CardContent>
          ...
          <div className="max-w-screen-xl mx-auto flex gap-3">
            {step === 5 ? (
              ...
              <Button onClick={handleNext} className="w-full h-12">
                Next
                createElement('ChevronRight', {className: 'ml-2 h-4 w-4'})
              </Button>
            )}
          </div>
        </div>
