 ...
 * Block ID: meditation
 * Category: component
 * Description: Meditation component
 * Runtime: Glue-Paste Safe (no imports, no JSX, no framework)
 */

(function () {
  // ---- Guard: prevent double definition ----
  if (window.__BLOCK_MEDITATION_DEFINED__) return;
  window.__BLOCK_MEDITATION_DEFINED__ = true;
  ...
  function mount(props = {}, host) {
    ...
    root.textContent = "Meditation component (basic conversion - JSX not fully parsed)";
  ...
  if (window.BlockRegistry && typeof window.BlockRegistry.register === "function") {
    window.BlockRegistry.register("meditation", {
      mount,
      unmount,
    });
  } else {
