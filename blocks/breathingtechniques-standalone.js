/**
 * Block ID: breathingtechniques
 * Category: component
 * Description: BreathingTechniques component
 * Runtime: Glue-Paste Safe (no imports, no JSX, no framework)
 */

(function () {
  // ---- Guard: prevent double definition ----
  if (window.__BLOCK_BREATHINGTECHNIQUES_DEFINED__) return;
  window.__BLOCK_BREATHINGTECHNIQUES_DEFINED__ = true;
  ...
  function mount(props = {}, host) {
    ...
    const difficulty = props.difficulty !== undefined ? props.difficulty : undefined;

    // ---- Root ----
    const root = document.createElement("div");
    root.style.padding = "2rem";
    root.style.fontFamily = "system-ui, sans-serif";
    root.textContent = "BreathingTechniques component (basic conversion - JSX not fully parsed)";
  ...
  if (window.BlockRegistry && typeof window.BlockRegistry.register === "function") {
    window.BlockRegistry.register("breathingtechniques", {
      mount,
      unmount,
    });
  } else {
