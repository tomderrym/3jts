 ...
 * Block ID: xpdisplay
 * Category: component
 * Description: XPDisplay component
 * Runtime: Glue-Paste Safe (no imports, no JSX, no framework)
 */

(function () {
  // ---- Guard: prevent double definition ----
  if (window.__BLOCK_XPDISPLAY_DEFINED__) return;
  window.__BLOCK_XPDISPLAY_DEFINED__ = true;
  ...
  function mount(props = {}, host) {
    ...
    const totalXP = props.totalXP !== undefined ? props.totalXP : undefined;
    ...
    root.textContent = "XPDisplay component (basic conversion - JSX not fully parsed)";
  ...
  if (window.BlockRegistry && typeof window.BlockRegistry.register === "function") {
    window.BlockRegistry.register("xpdisplay", {
      mount,
      unmount,
    });
  } else {
