...
const BlocksBuilder: React.FC = () => {
  ...
  return createElement('div', {className: 'space-y-3'}, '<header className="flex items-center justify-between gap-2">
        <div>
          createElement('h2', {className: 'text-xs font-semibold text-slate-100 tracking-tight'}, 'Blocks Builder')
          createElement('p', {className: 'text-[11px] text-slate-400'}, 'Mine your ideas into ordered blocks. Drag to rearrange or add new ones below.')
        </div>
      </header>

      <section className="rounded-xl border border-slate-800 bg-slate-900/70 p-3 space-y-2">
        createElement('h3', {className: 'text-[11px] font-medium text-slate-200 mb-1'}, 'Add Block')
        <form onSubmit={handleAddBlock} className="space-y-2">
          <div className="space-y-1">
            createElement('label', {className: 'block text-[11px] font-medium text-slate-300', htmlFor: 'block-title'}, 'Title')
            <input
              id="block-title"
              value={newTitle}
          ...
          <div className="space-y-1">
            createElement('label', {className: 'block text-[11px] font-medium text-slate-300', htmlFor: 'block-description'}, 'Description (optional)')
            <textarea
      ...
      <section className="rounded-xl border border-slate-800 bg-slate-900/70 p-3 space-y-2">
        <header className="flex items-center justify-between gap-2 mb-1">
          createElement('h3', {className: 'text-[11px] font-medium text-slate-200'}, 'Blocks')
          <div className="flex items-center gap-2">
            createElement('span', {className: 'text-[10px] text-slate-500'}, '{blocks.length} total')
            createElement('span', {className: 'text-[9px] text-slate-600', title: 'Blocks are automatically saved to localStorage (key: blocks_builder_blocks)'}, '💾')
          </div>
        </header>

        {blocks.length === 0 ? (
          createElement('p', {className: 'text-[11px] text-slate-400 text-center border border-dashed border-slate-700 rounded-lg py-4'}, 'No blocks yet. Add a block above to start your builder.')
        ) : (
              ...
              return (
                <li key={block.id} className="group">
                  ...
                  >
                    ...
                    >
                      createElement('span', {className: 'text-[9px] font-semibold text-slate-300'}, '{index + 1}')
                      <span className="flex flex-col gap-0.5">
                        createElement('span', {className: 'h-0.5 w-3 rounded-full bg-slate-500'})
                        createElement('span', {className: 'h-0.5 w-3 rounded-full bg-slate-500'})
                        createElement('span', {className: 'h-0.5 w-3 rounded-full bg-slate-500'}, null)
                    </button>

                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        createElement('p', {className: 'text-xs font-semibold text-slate-100 truncate'}, '{block.title}')
                        createElement('span', {className: 'text-[9px] px-2 h-6 inline-flex items-center rounded-full border border-slate-700/80 text-slate-300 bg-slate-900/80'}, '{block.type}')
                      </div>
                      createElement('p', {className: 'mt-1 text-[11px] text-slate-400 leading-snug line-clamp-3'}, '{block.description}')
                    </div>

                    <div className="flex flex-col justify-between items-end gap-1 pl-2">
                      createElement('button', {type: 'button'}, 'handleKeyboardMove(block.id, 'up')}
                        disabled={index === 0}
                      ...
                      >
                        Up')
                      createElement('button', {type: 'button'}, 'handleKeyboardMove(block.id, 'down')}
