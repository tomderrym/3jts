...
function DropZone({ onDrop }) {
  ...
  return (
    <div 
      ref={drop}
      className={`p-4 border-2 border-dashed ${isOver ? 'border-blue-500' : 'border-white/20'}`}
    >
      Drop here
    </div>
  );
    ...
    <>
      ...
      {show && (
        <div
          ...
          style={styles.popper}
          {...attributes.popper}
          className="bg-slate-800 px-3 py-2 rounded text-white z-50"
        >
          {content}
        </div>
      )}
...
>
  <div className="p-4">
    <p className="text-white">Resizable content</p>
  </div>
</Resizable>
```
