/**
 * Block ID: meditations
 * Category: component
 * Description: MEDITATIONS component
 * Runtime: Glue-Paste Safe (no imports, no JSX, no framework)
 */

(function () {
  // ---- Guard: prevent double definition ----
  if (window.__BLOCK_MEDITATIONS_DEFINED__) return;
  window.__BLOCK_MEDITATIONS_DEFINED__ = true;
  ...
  function mount(props = {}, host) {
    ...
    root.textContent = "MEDITATIONS component (basic conversion - JSX not fully parsed)";
  ...
  if (window.BlockRegistry && typeof window.BlockRegistry.register === "function") {
    window.BlockRegistry.register("meditations", {
      mount,
      unmount,
    });
  } else {
