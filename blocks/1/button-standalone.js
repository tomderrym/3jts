...
(function () {
  ...
  if (window.__BLOCK_BUTTON_DEFINED__) return;
  window.__BLOCK_BUTTON_DEFINED__ = true;
  ...
  function mount(props = {}, host) {
    ...
    const tmp = props.tmp !== undefined ? props.tmp : undefined;
    const Button = props.Button !== undefined ? props.Button : ({ children };

    // ---- Root ----
    const root = document.createElement("div");
    root.style.padding = "2rem";
    root.style.fontFamily = "system-ui, sans-serif";
    root.textContent = "Button component (basic conversion - JSX not fully parsed)";
  ...
  if (window.BlockRegistry && typeof window.BlockRegistry.register === "function") {
    window.BlockRegistry.register("button", {
      mount,
      unmount,
    });
  } else {
