 ...
 * Block ID: themes
 * Category: component
 * Description: THEMES component
 * Runtime: Glue-Paste Safe (no imports, no JSX, no framework)
 */

(function () {
  // ---- Guard: prevent double definition ----
  if (window.__BLOCK_THEMES_DEFINED__) return;
  window.__BLOCK_THEMES_DEFINED__ = true;
  ...
  function mount(props = {}, host) {
    ...
    const id = props.id !== undefined ? props.id : undefined;
    const config = props.config !== undefined ? props.config : undefined;

    // ---- Root ----
    const root = document.createElement("div");
    root.style.padding = "2rem";
    root.style.fontFamily = "system-ui, sans-serif";
    root.textContent = "THEMES component (basic conversion - JSX not fully parsed)";
  ...
  if (window.BlockRegistry && typeof window.BlockRegistry.register === "function") {
    window.BlockRegistry.register("themes", {
      mount,
      unmount,
    });
  } else {
