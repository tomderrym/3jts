 ...
 * Block ID: routines
 * Category: component
 * Description: Routines component
 * Runtime: Glue-Paste Safe (no imports, no JSX, no framework)
 */

(function () {
  // ---- Guard: prevent double definition ----
  if (window.__BLOCK_ROUTINES_DEFINED__) return;
  window.__BLOCK_ROUTINES_DEFINED__ = true;
  ...
  function mount(props = {}, host) {
    ...
    root.textContent = "Routines component (basic conversion - JSX not fully parsed)";
  ...
  if (window.BlockRegistry && typeof window.BlockRegistry.register === "function") {
    window.BlockRegistry.register("routines", {
      mount,
      unmount,
    });
  } else {
