 ...
 * Block ID: heavymodal
 * Category: component
 * Description: HeavyModal component
 * Runtime: Glue-Paste Safe (no imports, no JSX, no framework)
 */

(function () {
  // ---- Guard: prevent double definition ----
  if (window.__BLOCK_HEAVYMODAL_DEFINED__) return;
  window.__BLOCK_HEAVYMODAL_DEFINED__ = true;
  ...
  function mount(props = {}, host) {
    ...
    root.textContent = "HeavyModal component (basic conversion - JSX not fully parsed)";
  ...
  if (window.BlockRegistry && typeof window.BlockRegistry.register === "function") {
    window.BlockRegistry.register("heavymodal", {
      mount,
      unmount,
    });
  } else {
