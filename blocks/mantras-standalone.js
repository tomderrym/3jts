 ...
 * Block ID: mantras
 * Category: component
 * Description: MANTRAS component
 * Runtime: Glue-Paste Safe (no imports, no JSX, no framework)
 */

(function () {
  // ---- Guard: prevent double definition ----
  if (window.__BLOCK_MANTRAS_DEFINED__) return;
  window.__BLOCK_MANTRAS_DEFINED__ = true;
  ...
  function mount(props = {}, host) {
    ...
    root.textContent = "MANTRAS component (basic conversion - JSX not fully parsed)";
  ...
  if (window.BlockRegistry && typeof window.BlockRegistry.register === "function") {
    window.BlockRegistry.register("mantras", {
      mount,
      unmount,
    });
  } else {
