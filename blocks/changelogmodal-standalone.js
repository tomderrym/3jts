...
export default function ChangeLogModal: React.FC<ChangeLogModalProps> = ({ entries, onClose, onUpdate, onRestore }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-[#111114] border border-white/10 w-full max-w-3xl max-h-[85vh] rounded-3xl flex flex-col shadow-2xl overflow-hidden scale-in-center">
        ...
        <div className="p-6 border-b border-white/5 flex items-center justify-between shrink-0 bg-gradient-to-r from-indigo-500/10 to-transparent">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-white tracking-tight">Deployment History</h2>
              <p className="text-[10px] text-slate-500 uppercase tracking-widest font-black">AI App Builder Playground v2.4</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          {isAdding && (
            <div className="bg-white/5 border border-indigo-500/30 rounded-2xl p-4 animate-in slide-in-from-top-4 mb-8">
               <h3 className="text-sm font-bold text-white mb-4">New Log Entry</h3>
              ...
              <div key={idx} className="relative pl-8 border-l border-white/5 animate-in slide-in-from-left-4 duration-500" style={{ animationDelay: `${idx * 100}ms` }}>
                ...
                <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-md font-bold text-white">{entry.title}</h3>
                    </div>
                    {entry.checkpoint && onRestore && (
                        ...
                        >
                            <RotateCcw size={10} /> RESTORE CHECKPOINT
                        </button>
                    )}
                </div>
                  ...
                  <div className="bg-black/40 rounded-xl border border-white/5 overflow-hidden mb-4">
                    <div className="px-4 py-2 bg-white/5 border-b border-white/5 flex items-center justify-between">
                      <div className="flex items-center gap-2 text-[10px] text-slate-500 font-bold uppercase">
                        <Code2 size={12} /> Implementation Detail
                      </div>
                    </div>
                    <pre className="p-4 overflow-x-auto font-mono text-[11px] text-indigo-200/70 leading-relaxed">
                      {entry.snippet}
                    </pre>
                  </div>
                )}
                ...
                <div className="flex items-center gap-2 bg-white/5 rounded-lg px-3 py-2 border border-white/5">
                  ...
                  <span className="text-[10px] font-mono text-slate-500">{entry.remarks}</span>
                </div>
              </div>
            ))
          )}
        ...
        <div className="p-6 bg-black/20 border-t border-white/5 text-center shrink-0">
          <p className="text-[10px] text-slate-600 font-bold uppercase tracking-[4px]">Verified Deployment Chain</p>
        </div>
      </div>
    </div>
  );
