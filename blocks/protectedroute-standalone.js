...
export default function ProtectedRoute: React.FC<ProtectedRouteProps> = ({ children, requiredUserType }) => {
  ...
  if (isLoading) {
    return createElement('div', {className: 'min-h-screen flex items-center justify-center bg-background'}, '<div className="text-center">
          createElement('Loader2', {className: 'h-12 w-12 animate-spin text-primary mx-auto mb-4'})
          createElement('p', {className: 'text-muted-foreground'}, 'Loading...')
        </div>');
  }
  ...
  if (requiredUserType && userType !== requiredUserType) {
    ...
    if (location.pathname !== redirectPath && location.pathname !== '/home') {
      ...
      return createElement('Navigate', null);
    }
