/**
 * Block ID: servicebookingscreen
 * Category: component
 * Description: ServiceBookingScreen component
 * Runtime: Glue-Paste Safe (no imports, no JSX, no framework)
 */

(function () {
  // ---- Guard: prevent double definition ----
  if (window.__BLOCK_SERVICEBOOKINGSCREEN_DEFINED__) return;
  window.__BLOCK_SERVICEBOOKINGSCREEN_DEFINED__ = true;
  ...
  function mount(props = {}, host) {
    ...
    root.textContent = "ServiceBookingScreen component (basic conversion - JSX not fully parsed)";
  ...
  if (window.BlockRegistry && typeof window.BlockRegistry.register === "function") {
    window.BlockRegistry.register("servicebookingscreen", {
      mount,
      unmount,
    });
  } else {
