/**
 * Block ID: authcontext
 * Category: component
 * Description: AuthContext component
 * Runtime: Glue-Paste Safe (no imports, no JSX, no framework)
 */

(function () {
  // ---- Guard: prevent double definition ----
  if (window.__BLOCK_AUTHCONTEXT_DEFINED__) return;
  window.__BLOCK_AUTHCONTEXT_DEFINED__ = true;
  ...
  function mount(props = {}, host) {
    ...
    const email = props.email !== undefined ? props.email : undefined;
    const password = props.password !== undefined ? props.password : undefined;

    // ---- Root ----
    const root = document.createElement("div");
    root.style.padding = "2rem";
    root.style.fontFamily = "system-ui, sans-serif";
    root.textContent = "AuthContext component (basic conversion - JSX not fully parsed)";
  ...
  if (window.BlockRegistry && typeof window.BlockRegistry.register === "function") {
    window.BlockRegistry.register("authcontext", {
      mount,
      unmount,
    });
  } else {
