...
export function evaluateFeature(input: {
  ...
  if (input.importance_level === "critical" && !input.test_passed) return "REJECT";
  if (input.performance_score < 6) return "OPTIMIZE";
  if (input.success_rate > 0.9) return "PROMOTE_TO_DEFAULT";
  if (input.usage_frequency > 50) return "STANDARDIZE";
  return "REVIEW";
}
...
export function routeDiffForLearning(
  diff: DiffItemPayload,
  handlers: Partial<Record<DiffKind, DiffLearningHandler>>
): void {
