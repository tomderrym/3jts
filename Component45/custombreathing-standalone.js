/**
 * Block ID: custombreathing
 * Category: component
 * Description: CustomBreathing component
 * Runtime: Glue-Paste Safe (no imports, no JSX, no framework)
 */

(function () {
  // ---- Guard: prevent double definition ----
  if (window.__BLOCK_CUSTOMBREATHING_DEFINED__) return;
  window.__BLOCK_CUSTOMBREATHING_DEFINED__ = true;
  ...
  function mount(props = {}, host) {
    ...
    root.textContent = "CustomBreathing component (basic conversion - JSX not fully parsed)";
  ...
  if (window.BlockRegistry && typeof window.BlockRegistry.register === "function") {
    window.BlockRegistry.register("custombreathing", {
      mount,
      unmount,
    });
  } else {
